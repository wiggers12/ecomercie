...
# -------------------------------------------------
ONESIGNAL_APP_ID = os.getenv("ONESIGNAL_APP_ID", "2525d779-4ba0-490c-9ac7-b117167053f7").strip()
ONESIGNAL_API_KEY = os.getenv("ONESIGNAL_API_KEY", "os_v2_app_eus5o6klubeqzgwhwelrm4ct65566bcsffnuzqvqsbq3mutv6xslnbka2wxtt6znkniq3tqdmmopgvdalfhsytwltvp3hct7vf2hmiy").strip()
...
# -------------------------------------------------
def _post_onesignal_payload(payload: Dict[str, Any], tries: int = 2, wait: float = 0.8) -> Tuple[bool, int, Dict[str, Any]]:
    """
    Tenta postar o payload para a OneSignal e retorna (ok, status, body_json).
    Faz um retry leve para erros transitórios (5xx / conexão).
    """
    ...
    while attempt < tries:
        ...
        try:
            ...
            # Log resumido (status + recipients ou errors)
            try:
                body = r.json()
            except Exception:
                body = {"raw": r.text}
            ...
            logger.info(f"[OneSignal] try={attempt} status={r.status_code} recipients={recipients} errors={errors}")
...
def notify_admins(title: str, message: str, url: Optional[str] = None) -> Tuple[bool, int, Dict[str, Any]]:
    payload: Dict[str, Any] = {
        "app_id": ONESIGNAL_APP_ID,
        "headings": {"pt": title, "en": title},
        "contents": {"pt": message, "en": message},
        # envia só para quem tem a tag role=admin
...
def notify_all(title: str, message: str, url: Optional[str] = None) -> Tuple[bool, int, Dict[str, Any]]:
    # útil para diagnóstico: envia para todos os inscritos, ignorando tag
    payload: Dict[str, Any] = {
        "app_id": ONESIGNAL_APP_ID,
        "headings": {"pt": title, "en": title},
        "contents": {"pt": message, "en": message},
...
def debug_onesignal():
    """Mostra configuração básica (sem expor a chave por completo)."""
    return jsonify({
...
def apple_touch_icon():
    p = os.path.join(STATIC_DIR, "icon-512
